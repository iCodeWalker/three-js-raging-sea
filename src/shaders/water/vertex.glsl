...
void main() {
    ...
    // Speed of waves is too fast, we can slow it down by multiplying it with 0.2
    // elevation += cnoise(vec3(modelPosition.xz, uTime * 0.2));

    // Frequency is too small, multiply modelPosition.xz by 3.0
    // elevation += cnoise(vec3(modelPosition.xz * 3.0, uTime * 0.2));

    // The waves are too high, Multiply the noise by 0.15
    // elevation += cnoise(vec3(modelPosition.xz * 3.0, uTime * 0.2)) * 0.15;

    // Realistic waves have rounded troughs (the below part) and high crests,
    // we can use abs(...) function for that
    // elevation -= abs(cnoise(vec3(modelPosition.xz * 3.0, uTime * 0.2)) * 0.15);

    // Using for loop for combinging multiple perlin noise for better chaotic and raging pattern
